...
export interface InputProps {
  type?: 'text' | 'email' | 'tel' | 'number' | 'search' | 'date' | 'time' | 'password';
  label?: string;
  placeholder?: string;
  value?: string | number;
...
export function Input({
  ...
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontFamily: 'var(--font-sans)', width: '100%', ...style }}>
      {label && (
        <span style={{ fontSize: 13, fontWeight: 'var(--text-label-weight)' as unknown as number, color: 'var(--text-heading)' }}>{label}</span>
      )}
