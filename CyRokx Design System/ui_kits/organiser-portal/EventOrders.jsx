const { Icon, Input, Button, Badge } = window.CyRokxDesignSystem_ef2ebf;
...
function EventOrders() {
  ...
  return (
    <div>
      <OrdPanelHead title="Orders" subtitle={`${ORDERS.filter((o) => o.status === 'Completed').length} completed · ${ORDERS.filter((o) => o.status === 'Incomplete').length} incomplete`}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18, alignItems: 'flex-end' }}>
        <div style={{ flex: 1 }}><Input label={null} icon="search" placeholder="Search name or email" value={kw} onChange={(e) => setKw(e.target.value)} /></div>
        <div style={{ width: 240 }}><Input label={null} placeholder="Registration ID" value={rid} onChange={(e) => setRid(e.target.value)} /></div>
