const { Icon, Input, Select, Textarea, Switch, Button, Badge } = window.CyRokxDesignSystem_ef2ebf;
...
function EventDiscounts() {
  ...
  const blank = { id: 'new', code: '', desc: '', cat: DISC_CATS[0], start: '', end: '', pct: '0', abs: '0', max: '100', claimed: 0, enabled: true };
  ...
  return (
    <div>
      <DiscPanelHead title="Discounts" subtitle="Codes buyers enter at checkout, or auto-applied offers. Leave the code blank to auto-apply.">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 20, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {list.map((d) => {
            ...
            return (
              <button key={d.id} onClick={() => setActiveId(d.id)} style={{ textAlign: 'left', cursor: 'pointer', background: 'var(--surface-card)', border: `1.5px solid ${on ? 'var(--color-accent)' : 'var(--border-default)'}`, borderRadius: 'var(--radius-card)', padding: 16, boxShadow: on ? 'var(--shadow-card)' : 'none' }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginBottom: 10 }}>
                  {d.pct !== '0' && d.pct ? `${d.pct}% off` : ''}{d.pct !== '0' && d.pct && d.abs !== '0' && d.abs ? ' · ' : ''}{d.abs !== '0' && d.abs ? `₹${d.abs} off` : ''} · {d.cat}
                </div>
        ...
        <div style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-card)', padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 6, borderTop: '1px solid var(--border-default)' }}>
              ...
              <Button><Icon name="save" size={15} />Save discount</Button>
            </div>
          </div>
        </div>
      </div>
